...
func RateLimitMiddleware(c *gin.Context) {
    if !config.Env.RateLimit.Enabled {
        c.Next()
        return
    }
    ...
    c.Header("X-Rate-Limit-Limit", fmt.Sprintf("%v", config.Env.RateLimit.Capacity))
    c.Header("X-Rate-Limit-Duration", fmt.Sprintf("%v", config.Env.RateLimit.FillInterval))
